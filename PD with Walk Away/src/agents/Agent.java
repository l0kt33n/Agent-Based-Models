...
public class Agent implements Steppable {
	...
	//agents.Agent.StrategySet.StrategySet(Action action, boolean moveCooperate, boolean moveDefect, boolean moveNothing)
	...
	private void reproduce() {
		if(!sim.isLocalReproduction())
		{
			...
			Agent o = sim.makeAgent(strategy);
		...
		if(sim.isLocalReproduction())
		{
			...
			int Radius = sim.getReproductionRadius();	
			Agent o = sim.makeAgent(this.strategy,Radius, this.x, this.y);
			double split = resources / 2;
			o.resources = resources - split;
			resources = split;
		}
