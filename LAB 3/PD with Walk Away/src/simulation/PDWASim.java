...
public class PDWASim extends SimStateSparseGrid2D {
	...
	private int nRealisticTFT = 100;
	private int playRadius = 1;
	private boolean localReproduction = true;
	...
	protected void makeAgents() {
		populationCap = nNaiveC + nNaiveD + nWalkawayC + nWalkawayD + nTFTStationary + nTFTMobile + nPAVLOVStationary + nPAVLOVMobile + nRealisticTFT;	// we freeze this so it can't be changed while running
	...
	public Agent makeAgent(Strategy strat, int radius, int x1, int y1)
	{

		Int2D ints = getEmptyLocationNear(x1,y1,radius);
		if (ints == null)
		{
			return null;
		}
	...
	/**
	  * Returns a random, empty location within <i>radius</i> units of the given (<i>x</i>, <i>y</i>) location, or null
	  * if there are no empty locations nearby. Can return the location (<i>x</i>, <i>y</i>) if it is empty.
	...
	public Int2D getEmptyLocationNear(int x, int y, int radius) {
	    ...
	    Collections.shuffle(selector); // NOTE NOTE NOTE! This does not use the MASON random number generator,
	                                   // which means this isn't replicable, but we will let it go for simplicity
	    for (int i = 0; i < size; i++) { // we step through the now randomly-ordered list of indexes into the available
	                                     // locations and return the first empty one
	        test.clear();
	...
	private RGBTColor colorByStrategy(Strategy strat) {
		switch (strat) {
			...
			case REALISTIC_TFT:
				return new RGBTColor(1, 0.5, 0.5, 1);
			default:
		}
